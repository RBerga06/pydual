...
class DynDualPart[S: Shape](DualPart[DynDualBasis, S]):
    ...
    def cov(self, rhs: Self, /) -> Tensor[S]:
        ...
        return np.sum(
            [self.data[k].cov(rhs.data[k]) for k in self.data.keys() & rhs.data.keys()],
            axis=0,
    ...
    def map_[Z: Shape](
        ...
        return DynDualPart(
            {k: v.map_(f_scalar, f_vector) for k, v in self.data.items()}
        )
    ...
    def map2_[S2: Shape, Z: Shape](
        ...
        def callback[N: int](
            x: DualPart[DualBasis, S] | None, y: DualPart[DualBasis, S2] | None, /
        ) -> DualPart[DualBasis, Z]:
            if x is None:
        ...
        return DynDualPart(
            {
                k: callback(self.data.get(k), rhs.data.get(k))
                for k in self.data.keys() | rhs.data.keys()
            }
        )
    ...
    def map2(self, rhs: Self, f: Callback2[S], /) -> Self:
        # TODO: Find a way of changing output shape
        def callback[N: int](
            x: DualPart[DualBasis, S] | None, y: DualPart[DualBasis, S] | None, /
        ) -> DualPart[DualBasis, S]:
            if x is None:
                assert y is not None
    ...
    def map2alt(
        self, rhs: Self, f: Callback2[S], fl: Callback1[S], fr: Callback1[S], /
    ) -> Self:
        """Apply the given functions to all dual parts in `self` and `rhs`."""
        # TODO: Find a way of changing output shape
        return type(self)(
            {
                k: self.data[k].map2(rhs.data[k], f)
                for k in self.data.keys() & rhs.data.keys()
            }
